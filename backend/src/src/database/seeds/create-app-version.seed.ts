...
export default class CreateAppVersionSeeder implements Seeder {
  ...
  public async run(dataSource: DataSource): Promise<void> {
    await dataSource.query('TRUNCATE TABLE app_versions RESTART IDENTITY CASCADE;');
    ...
    await repository
      ...
      .then(() => {
        console.warn('Database seeding: Application version data inserted successfully.');
      })
      .catch(error => {
        console.warn('Database seeding: Application version data insertion failed.');
        console.warn(error);
      });
  }
}
